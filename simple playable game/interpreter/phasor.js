...
// Website wrapper for starting the Phasor AGI intepreter
var Phasor =
{
  ...
  net: function(path, multiplayerEnabled) {
    ...
    Phasor.path = path;
    if (!window.PICTURES)
      Phasor.loadResource(path + '/game.js');
    Phasor.initViewCss();
    Phasor.initPictureCss();
    Phasor.initHTML();
  ...
  initHTML: function() {
    document.getElementById('Phasor').innerHTML = '<div id="canvas"><div id="dialog"><div id="border"></div></div></div>';
  },
  ...
  updateAddressBar: function(roomNr) {
    if (Hacks.updateAddressBarAllowed(AGI.game_id, roomNr))
      document.location.hash = Phasor.getRoomName(roomNr);
  },
